...
fn isOverlap(ranges: *ArrayList(Range)) !bool {
    ...
    if (first.start <= second.start and first.end >= second.start) {
        ...
        return true;
    } else if (first.start <= second.end and first.end >= second.end) {
        try std.io.getStdOut().writer().print("{d}-{d},{d}-{d}\n", .{ first.start, first.end, second.start, second.end });
        return true;
    } else if (second.start <= first.start and second.end >= first.start) {
        try std.io.getStdOut().writer().print("{d}-{d},{d}-{d}\n", .{ first.start, first.end, second.start, second.end });
        return true;
    } else if (second.start <= first.end and second.end >= first.end) {
        try std.io.getStdOut().writer().print("{d}-{d},{d}-{d}\n", .{ first.start, first.end, second.start, second.end });
        return true;
    }
